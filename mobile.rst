...
"gravity" - выравнивание текста left/right/center. По умолчанию center.
...
"clicked" - надпись активная для нажатия, порождает событие onInput при нажатии (с listener=id элемента)
...
Функции (python и NodaScript) для работы с s3
...
Используются для выбора конкретного значения узла на форме.

При выборе используется представление. Представление объекта задается в поле Шаблон записи в классе. Там также можно использовать html.
Также, представление значения при выборе значения храниться в переменная_view в _data. И наоборот, если там есть значение, то оно будет использовано как представение. 

При выборе узла, возникает событие onInput с listener=id поля. В _data попадают _id выбранного узла (в ключ=id эдемента) а также ``<id поля>_data`` - _data выбранного узла ,  ``<id поля>_view`` - представление выбранного узла

"type" : "NodeInput"

"dataset" – имя класса узла

"caption" – заголовок поля

"value" – значение по умолчанию. Задается в виде стандартной ссылки на узел.

Вывод представления узла NodeLink
"""""""""""""""""""""""""""""""""""""""""""

"type" : "NodeLink"

"value" - Значение в нормализованном формате

Выводит представление по ссылке, переданное в value. Ссылка должна быть в стандартном виде (<uid_конфигурации>$<имя класса>$<id>). Представление должно быть задано в шаблоне записи (на закладке Отображение), иначе будет выведен _id

В остальном, поля соответствуют надписи Text
...
Вывод представления датасета DatasetLink
"""""""""""""""""""""""""""""""""""""""""""
...
Выводит представление по ссылке, переданное в value. Ссылка должна быть в стандартном виде (<имя_датасета>$<id>). Представление должно быть задано в шаблоне, иначе будет выведен _id
...
Тема выбирается пользователем в настройках. Платформа отрисовывается как надо, но элементы, заданные пользователем в разметке, могут содержать цвета, установленные непосредственно. И вот эти цвета в темной теме тоже должны быть приятны глазу. Поэтому для этих целей во всех элементах где 
...
Для того чтобы у узла в обложке отрисивался тег(теги) в _data достаточно поместить массив тегов (массив на случай если тегов>1) в ключ ``"_tags":["my_tag1","my_tag2"]``  и в обложке появятся теги внизу. Цвет тега генерится по md5-хешу (одинаковый алгоритм в мобильном и веб-клиенте), цвет текста подстраивается

Если не надо автоматический цвет то можно задать явно в формате ``_tags:[{"id":"mytag","color":"#FFF555"}]``

Для веб-клиента можно включить галочку «Отображать облако тегов» и добавляется облако-фильтр по тегам, по которому можно делать отбор.
