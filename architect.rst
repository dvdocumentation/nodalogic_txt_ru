...
Данные из интерфейса и обработчиков попадают в _data, и наоброт хранимые в _data данные оторажаются в интерфейсе. По сути - это обычная JSON-ориентированная NoSQL.

На мобильном устройстве при пользовательском вводе возникает событие onInput  и если на него есть какой то метод-обработчик, то он обработает эти данные, сохранит, возможно вызовет другие методы  или отправит другим узлам (путем вызова их методов). На картинке ниже ситуация: пользователь нажал на кнопку - сработало событие onInput, на котрое есть подписка в виде метода Input (python) - метод python (на мобильном устройстве) выводит тост(сообщение) с переменной input1, в которой хранится ввод из поля ввода.
