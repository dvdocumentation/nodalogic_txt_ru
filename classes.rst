...
5. _sum_transaction_unique(self, scheme_name: str, *,  unique_key: str,  period: str,  keys: list,  values: list, meta: dict | None = None, ) - добавляет уникальную (идемпотентную) суммовую транзакцию, которая не может быть изменена при последующих вызовах метода (кроме специальных методов)

6. _remove_sum_transaction_unique(self, scheme_name: str, *, unique_key: str) - удаляет уникальную транзакцию по ключу

7. _rebuild_sum_transactions(self, scheme_name: str) - полный пересчет схем
